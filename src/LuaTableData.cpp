...
std::vector<LuaTableData::_AppendBaseClassFunc> LuaTableData::_append_fns{
    _appendBaseClass<SSS::GL::ModelBase>,
    _appendBaseClass<SSS::GL::RendererBase>,
    _appendBaseClass<SSS::GL::PlaneRendererBase>
};

bool LuaTableData::recursive{ false };

LuaTableData::LuaTableData(sol::state& lua, sol::table table, sol::environment env,
    std::string name)
    : LuaTableData(sol::type::table, name)
{
    for (auto const& [k, v] : table) {
        ...
        emplace(key, LuaTableData(type, new_name));

        if (recursive && (type == sol::type::table || type == sol::type::userdata))
            at(key)._append(LuaTableData(lua, v, env, new_name));
    }
...
void LuaTableData::_appendUserdata(sol::state& lua, sol::environment env, sol::userdata u)
{
    _append(LuaTableData(lua, u[sol::metatable_key], env, name));
    for (auto& [k, v] : *this) {
...
template <class T>
static void LuaTableData::_appendBaseClass(sol::state& lua, LuaTableData& data,
...
std::string LuaTableData::to_string() const
{
    static int layer = -1;
    ++layer;
    std::string ret;
